...
`list_migrations` (prod, today) confirms `v0.56` + `v0.61` realtime-terminal-authz **are applied**,
but **`v0.65` r0_token_least_privilege is NOT applied** (matches `feedback_f1_f8_hardening`:
"migrate_v0.65 NOT applied; custom-role E2E gate before cutover"). `mint-realtime-token` currently
issues **no live tokens** and `realtime_private_enabled` is **OFF in prod** pending the owner-run S7
cutover. So a desktop R0 host would broadcast to a channel **no viewer can privately join yet**. R0
work must be **phased behind the Mac cutover** and ship **default-OFF with proven byte-identical
...
`Windsurf`, `OpenCode Go`, `Grok`. HTTP ports cross-platform via **manually-pasted session
cookies/env** (only macOS `SweetCookieKit` auto-import is unavailable) — ship manual-cookie with
clear help text + expiry surfaced. *(Reuse `openrouter.rs` `$1=100_000` credit SCALE for
...
*(Deferred provider group: JetBrains AI, Augment, Kiro, OpenCode, Droid/Factory, Antigravity,
Volcano Engine, Synthetic — Mac itself deferred these; OS-specific credential-store scraping / helper
backend the desktop lacks. Lowest ROI; do the ~35 api-key/cookie providers first. §6.)*
...
**v0.18.0 — System Monitor: cross-platform sensor metrics into `p_metrics`** · effort **L**.
- **First-principles:** macOS SMC/IOReport/HID are Apple-only; the Windows/Linux equivalent is
  `sysinfo` `Components` (hwmon/WMI temps) + `starship-battery`. Fill the existing 19 `device_sensors`
  columns from the platform's real backend — **the whole data plane + validation already exist
  server-side (v0.63/v0.64)** — capability-gated, ship only the subset the platform can read.
- **Wire invariant:** send ONLY v0.63 schema keys; `battery_state` MUST be exactly
  ...
  `Full` variant is silently dropped; this is the #1 trap); temps are °C reals dropped outside
  `-40..150`; **OMIT `thermal_state`** (no desktop analog — don't invent); capability map must be
  **truthful** (never fabricate a temp when `Components` is empty on a Windows VM); blob ≤ 8192 B or
  v0.64 ignores the whole block.
...
- ⚠️ **Acceptance criterion, not just an invariant (critic MEDIUM + §1b):** server RLS/RPC/edge-fn
  ...
  parse the start-payload `realtime_private` with `#[serde(default)] bool` → **fail-closed to
  public/no-broadcast**, with **proven byte-identical no-broadcast behavior**. Do not build against
  a moving token contract — stay **agnostic to token internals** so a v0.65 custom-role change can't
  break the producer. **Sequence this after the Mac R0 cutover lands.**
...
v0.14.0  Service-status + search (trimmed) .... v1.30/UX (M)
v0.15.0  API-key batch 1 (z.ai/GLM/DeepSeek/Crof)
v0.16.x  API-key batches 2 + 3 (+7 critic) + Bedrock
v0.17.0  helper_heartbeat + device read-back ... backend adopt (M)
v0.18.0  System Monitor sensors ................ v1.38 (L)
v0.19.0  Onboarding + account + swarm viewer ... first-run (M)
v0.20.0  Cookie/manual-paste collectors ........ (L)
v0.21.0  Vertex AI ............................. (M)
v0.24.0  Managed Codex on-plan ................. v1.35-36 (L)  [after §5 Gemini decision N/A to Codex]
v0.22.0  Terminal foundations .................. v1.31-34 (L)
...
```
Numbers are indicative; batches may split further under review. Every ship: implement → `cargo
fmt`/`clippy`/tests green → **CI + launch-smoke green** → Codex/Gemini diff review → tag → prerelease
→ smoke-verify launch+mount → promote `--latest`. `PROJECT_FIX_*.md` per real bug.
...
- **Scanner model tables + cost algorithm** — ALREADY BIT-EXACT; family-fallback (v0.11.1) was the
  sole drift.
- **Provider group JetBrains AI / Augment / Kiro / OpenCode / Droid(Factory) / Antigravity / Volcano
  Engine / Synthetic** — Mac itself deferred; OS-specific credential-store scraping / helper backend
  the desktop lacks. Lowest ROI.
...
- **R0 broadcast** — redact before sink, std-base64, lowercase `pterm:`, event allowlist,
  `private:true` per message, `realtime_private` `#[serde(default)]` fail-closed, proactive token
  refresh, output-only, default-OFF byte-identical.
