...
pub struct ProviderCreds {
    ...
    /// Venice api-key (Bearer). Secret — masked in `ProviderCredsView`.
    #[serde(default)]
    ...
    /// Perplexity session cookie (not an api-key). Secret — masked in
    /// `ProviderCredsView`.
    #[serde(default)]
    ...
    /// StepFun `Oasis-Token` cookie (not an api-key). Secret — masked in
    /// `ProviderCredsView`.
    #[serde(default)]
    ...
    /// Kimi (kimi.com) `kimi-auth` JWT — raw token or a Cookie header. Secret —
    /// masked in `ProviderCredsView`.
    #[serde(default)]
    pub kimi_auth_token: Option<String>,
    /// Grok (grok.com) session cookie (not an api-key). Secret — masked in
    /// `ProviderCredsView`.
    #[serde(default)]
    ...
    /// Volcano Engine (Ark) api-key (Bearer). Secret — masked in
    /// `ProviderCredsView`.
    #[serde(default)]
    ...
    /// Mistral session cookie (must contain `ory_session_*`). Secret — masked
