...
struct UsageResponse {
    ...
    /// Internal codename for "Designs". Anthropic occasionally serves the
    /// key as JSON null (account on the rollout but no usage yet); the
    /// Mac path treats present-but-null as a 0-utilization bucket. We
    /// don't replicate that nuance — we silently drop nulls. Worst case
    /// the bar doesn't show until the user actually uses Designs.
    #[serde(default)]
    iguana_necktie: Option<UsageWindow>,
    /// Internal codename for "Daily Routines". Same null semantics as
    /// `iguana_necktie`.
    #[serde(default)]
...
pub async fn collect_claude() -> Option<QuotaSnapshot> {
    let creds = match read_credentials() {
        ...
        None => {
            log::debug!(
                "Claude .credentials.json absent or unparseable — skipping quota fetch"
            );
            return None;
        }
    };
...
mod tests {
    ...
    fn token_fresh_iso_string_future() {
        ...
        assert!(is_token_fresh(&creds_with_expiry(serde_json::Value::String(
            future
        ))));
    }
    ...
    fn token_fresh_epoch_ms_future() {
        ...
        assert!(is_token_fresh(&creds_with_expiry(serde_json::Value::Number(
            future_ms.into()
        ))));
    }
    ...
    #[test]
    fn format_plan_known_and_unknown() {
        assert_eq!(format_plan(Some("max_20x")), "Max 20x");
        assert_eq!(format_plan(Some("max_5x")), "Max 5x");
        assert_eq!(format_plan(Some("pro")), "Pro");
        assert_eq!(format_plan(Some("custom_enterprise")), "custom_enterprise");
        assert_eq!(format_plan(None), "Claude");
        assert_eq!(format_plan(Some("")), "Claude");
    }
