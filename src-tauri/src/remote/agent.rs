...
// v0.9.1a — use tauri's JoinHandle (re-exported via async_runtime
// module) to match the type returned by `tauri::async_runtime::spawn`
// at the bottom of this file. Tauri 2's JoinHandle is its own type,
// not the tokio one — using `tokio::task::JoinHandle` here would
// give a type mismatch on the AgentHandle.join field. (This was
// caught immediately by the compiler when fixing the v0.8.0
// `tokio::spawn` → `async_runtime::spawn` bug; documenting here so
// the next reader doesn't naively re-import tokio's JoinHandle.)
